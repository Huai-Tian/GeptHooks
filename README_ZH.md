...
每个核心持有两套 EPT 视图：*clean* 视图（恒等映射，原始字节）与 *hooked* 视图（钩子页改译到影子副本）。钩住执行 = 切换翻译，而不是陷入：**每次钩子命中零 VM-Exit**。实测：18 万余次 NtClose 拦截期间 EPT violation 计数始终为 0。
...
- **老 CPU 自动降级**
无 VMFUNC 的 CPU 上，钩子自动退回经典 EPT violation 方案（拆 2M 页 + X 位切换）——API 完全一致，你的代码零改动。

- **简洁的驱动友好 API**
在你自己的内核驱动里用几个 C 调用即可完成钩子的安装、移除、枚举与原函数调用——无需任何 hypervisor 背景知识。

- **MSR 拦截（MSR Bitmap）—— 规划中**
基础设施已就位（`VmxSetMsrRw`）；读伪造样例（如 IA32_LSTAR）在路线图上。
...
- **CPU**：支持 VT-x + EPT 的 Intel 处理器；**零 VM-Exit 路径需 VMFUNC（Haswell 及以上）**——更老的 CPU 自动走 violation 降级路径
...
| STAGE 2 | NtClose 全系统监控 | 多轮累计 100 万+次拦截，16–89 分钟稳定运行 |
...
核心路径（虚拟化、双 EPT VMFUNC 钩子、隐身、detour API）已在 i7-6700HQ（8 核，Windows 10 x64）上完成分阶段实测毕业。本框架为研究级质量：仍可能存在导致蓝屏的缺陷——请务必在可弃置的测试机上运行。
